...
# "country marine"~500 requires both to appear within 500 words — equivalent
# to co-occurrence within the same abstract, confirming marine framing and
# excluding pure agricultural or development literature from reef countries
# Split into two chunks to stay within OpenAlex URL length limits

# Anchor term — "marine" confirmed sufficient to scope to marine context
...
# Split into two chunks — each chunk stays under 1400 chars so that
# the full query (chunk + block2 + block3) stays within URL limits
geo_mid <- ceiling(length(geo_countries) / 2)

make_geo_block <- function(countries) {
  expand.grid(country = countries, anchor = geo_anchors,
...
block1_geo_a <- make_geo_block(geo_countries[1:geo_mid])
block1_geo_b <- make_geo_block(geo_countries[(geo_mid + 1):length(geo_countries)])
...
# Runs three sub-queries per intervention type:
#   query_eco:   ecosystem descriptors and named reef systems (block1_eco)
#   query_geo_a: first half of country list anchored to marine context
#   query_geo_b: second half of country list anchored to marine context
# Results are combined and deduplicated within each intervention type.
# Splitting block1_geo into two chunks keeps URL length within API limits.
...
run_search <- function(block3, intervention_label) {
  ...
  query_geo_a <- paste0("(", block1_geo_a, ") AND (", block2, ") AND (", block3, ")")
  query_geo_b <- paste0("(", block1_geo_b, ") AND (", block2, ") AND (", block3, ")")
  ...
  res_geo_a <- fetch_count(query_geo_a)
  res_geo_b <- fetch_count(query_geo_b)
  
  n_eco   <- if (!is.null(res_eco))   res_eco$count   else NA_integer_
  n_geo_a <- if (!is.null(res_geo_a)) res_geo_a$count else NA_integer_
  n_geo_b <- if (!is.null(res_geo_b)) res_geo_b$count else NA_integer_
  
  cat("  Ecosystem / named terms:  ", n_eco,   "\n")
  cat("  Geo chunk A + marine:     ", n_geo_a, "\n")
  cat("  Geo chunk B + marine:     ", n_geo_b, "\n")
  
  # Sum is a conservative upper bound — deduplication at retrieval handles overlap
  n_total <- sum(c(n_eco, n_geo_a, n_geo_b), na.rm = TRUE)
  ...
  if (retrieve) {
    ...
    raw_geo_a <- if (!is.na(n_geo_a) && n_geo_a <= max_records) {
      cat("  Retrieving geo chunk A...\n")
      fetch_records(query_geo_a, "geo_a")
    } else {
      cat("  Skipping geo chunk A — count", n_geo_a, "exceeds max_records.\n")
      NULL
    }
    
    raw_geo_b <- if (!is.na(n_geo_b) && n_geo_b <= max_records) {
      cat("  Retrieving geo chunk B...\n")
      fetch_records(query_geo_b, "geo_b")
    } else {
      cat("  Skipping geo chunk B — count", n_geo_b, "exceeds max_records.\n")
      NULL
    }
    
    raw <- bind_rows(raw_eco, raw_geo_a, raw_geo_b)
